      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <button
              ...
              className={`px-6 py-2.5 rounded-full text-base font-extrabold transition cursor-pointer border-2 ${
                selectedCategory === cat
                  ...
                  : 'bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 border-slate-400 dark:border-slate-700 hover:bg-slate-200 dark:hover:bg-slate-800'
              }`}
            >
              {cat}
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {filteredExercises.map((exercise) => (
            <div
              key={exercise.id}
              className="bg-white dark:bg-slate-900 rounded-2xl border-2 border-slate-300 dark:border-slate-800 shadow-md overflow-hidden hover:border-red-600 transition flex flex-col"
            >
              <div className="relative w-full aspect-video bg-slate-950 border-b-2 border-slate-900">
                ...
                <div className="absolute top-3 right-3 px-3 py-1 bg-white/90 dark:bg-slate-800 text-slate-950 dark:text-white text-xs font-black rounded border border-slate-900 dark:border-slate-700 backdrop-blur-sm pointer-events-none">
                  {exercise.difficulty}
                </div>
              </div>
              ...
              <div className="p-6 flex-grow flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-slate-800 dark:text-slate-300 text-base font-medium leading-relaxed mb-4">
                    {exercise.description}
                  </p>
                </div>

                <div className="pt-4 border-t-2 border-slate-200 dark:border-slate-800 flex items-center justify-between">
                  <span className="text-sm font-black text-red-950 dark:text-red-300 bg-red-100 dark:bg-red-950/60 border border-red-300 dark:border-red-800 px-3 py-1 rounded-full">
                    {exercise.category}
                  </span>
